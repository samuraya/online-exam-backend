...
class Stream implements StreamInterface
{
    ...
    public function isPipe(): bool
    {
        if ($this->isPipe === null) {
            $this->isPipe = false;
        //my own conditional check , php 7.4 requirement. False value has to be confirmed . Now I am not sure.
            $fstat = isset(fstat($this->stream)['mode']) ? fstat($this->stream)['mode'] : null;
        //************************************
            if ($this->stream) {
               $this->isPipe = ($fstat & self::FSTAT_MODE_S_IFIFO) !== 0;
            }
        }
